...
def caculate_fall_back_rate(current_pack_num,origin_pack_num):
  if current_pack_num>origin_pack_num:
    current_pack_num,origin_pack_num=origin_pack_num,current_pack_num
  return (origin_pack_num-current_pack_num)
...
def send(key,
  ...
  if sat_id_to<num_satellite:
    ...
    if delay!=None:
      ...
      satellite_queue[current_loop_id+math.ceil(delay/time_step)][sat_id_from][key]=satellite_queue[current_loop_id++math.ceil(delay/time_step)][sat_id_from].get(key,0)+loss_packet_num
...
def simulator(
      ...
      fall_back_percentage,
      queue_len,
      generate_length,
      start_emulate_id,
      last_call_queue_length 
    ):
  # print(len(queue_len))
  # print(queue_len)
  fall_back_flag=0
  ...
  for sat_id in range(0,num_satellite):
    satellite_store_queue[sat_id][start_emulate_id*time_step]= satellite_store_queue[sat_id].get(start_emulate_id*time_step,0)+last_call_queue_length[sat_id]
  for gid in range(0,num_ground_station):
    ground_station_store_queue[gid][start_emulate_id*time_step]=ground_station_store_queue[gid].get(start_emulate_id*time_step,0)+last_call_queue_length[gid+num_satellite]
  
  #####################################
  #             开始模拟               #
  #####################################
  current_routing_table_id=0
  current_loop_id=start_emulate_id
  total_packet_gen=0
  total_througput=0
  total_queue_len=0
  ...
  print("start emulate id=",start_emulate_id)
  for time_since_epoch in range(start_emulate_id*time_step,min((start_emulate_id+generate_length)*time_step,total_sim_time),time_step):
    print("Simulation progress: {:.2%}".format(time_since_epoch / total_sim_time))
    print("min((start_emulate_id+generate_length)*time_step",(start_emulate_id+generate_length),total_sim_time//time_step)
    now=time_since_epoch*u.ns+epoch
    # 计算当前模拟时间
    if time_since_epoch<total_generate_time_ns and (time_since_epoch>start_emulate_id*time_step or time_since_epoch==0) :
      ...
      # 生成数据包
    for satellite_id in range(0,num_satellite):
        current_queue_len=get_size(satellite_store_queue[satellite_id])
        if(caculate_fall_back_rate(current_queue_len,queue_len[current_loop_id][satellite_id])>3):
          print("current_queue_len",current_queue_len)
          print("predict queue len",queue_len[current_loop_id][satellite_id])
          fall_back_flag=1
    ...
    if current_loop_id<len(routing_table):
      for satellite_id in range(0,num_satellite): 
        Queue_size=get_size(satellite_store_queue[satellite_id])
        if routing_table[current_loop_id][satellite_id+1]!=[]:
          
          total_out_packets=0
          for i in routing_table[current_loop_id][satellite_id+1]:
    ...
    del a
    if(fall_back_flag==1 or current_loop_id-start_emulate_id>=generate_length):
      print("fall_back_flag=",fall_back_flag)
      print("generate length=",current_loop_id,generate_length)
      for current_id in range(current_loop_id,total_time_step+5,1):
        for satellite_id in range(0,num_satellite):
          for key in list(satellite_queue[current_id+1][satellite_id].keys()):
            satellite_store_queue[satellite_id][key]=satellite_store_queue[satellite_id].get(key,0)+satellite_queue[current_id+1][satellite_id][key]
        for ground_station_id in range(0,num_ground_station): 
          for key in list(ground_station_queue[current_id+1][ground_station_id].keys()):
            ground_station_store_queue[ground_station_id][key]=ground_station_store_queue[ground_station_id].get(key,0)+ground_station_queue[current_id+1][ground_station_id][key]
      break
    current_loop_id+=1
  latency=[]
  for satellite_id in range(0,num_satellite):
      current_queue_length.append(get_size(satellite_store_queue[satellite_id]))
  for gid in range(0,num_ground_station):
    current_queue_length.append(get_size(ground_station_store_queue[gid]))
  ...
  print("current queue len=",current_queue_length)
  print("current_loop_id=",total_sim_time//time_step,fall_back_flag,current_loop_id)
  print("fall_back_flag=",fall_back_flag)
  return throughput_table,latency,packet_droped_table,satellite_queue_len,ground_station_queue_len,current_loop_id,fall_back_flag or current_loop_id!=total_sim_time//time_step,current_queue_length
