...
def send(key,count,sat_id_from,sat_id_to,epoch,now,num_satellite,current_loop_id,satellite_queue,ground_station_queue,packet_droped_table,satellites,ground_stations,isl_packet_drop_rate,gsl_packet_drop_rate,time_step):
  delay=None
  if sat_id_to<num_satellite:
    ...
    if delay!=None:
      ...
      satellite_queue[current_loop_id+math.ceil(delay/time_step)][sat_id_to][key]=satellite_queue[current_loop_id+math.ceil(delay/1000)][sat_id_to].get(key,0)+count-loss_packet_num
    else:
...
def simulator(
  ...
  # 初始化时隙队列
  for i in range(0, total_time_step+100):
  ...
  for i in range(0, num_satellite):
  ...
  for time_since_epoch in range(0,total_sim_time,time_step):
    print("Simulation progress: {:.2%}".format(time_since_epoch / total_sim_time), end="\r")
    now=time_since_epoch*u.ns+epoch
    # 计算当前模拟时间
    for satellite_id in range(0,num_satellite):
      satellite_queue[current_loop_id][satellite_id][time_since_epoch]=satellite_queue[current_loop_id][satellite_id].get(time_since_epoch,0)+satellite_generated_packages_per_time_step
      # 生成数据包
    if routing_table[current_routing_table_id][0]<now:
      current_routing_table_id+=1
    for satellite_id in range(0,num_satellite): 
      if len(satellite_queue[current_loop_id][satellite_id])==0:
        continue
      ...
      if routing_table[current_routing_table_id][satellite_id+1]!=[]:
        ...
        for i in routing_table[current_routing_table_id][satellite_id+1]:
          ...
          for key in list(satellite_queue[current_loop_id][satellite_id].keys()):
            if out_count==0:
              break
            if satellite_queue[current_loop_id][satellite_id][key]<=out_count:
              count=satellite_queue[current_loop_id][satellite_id][key]
              del satellite_queue[current_loop_id][satellite_id][key]
              out_count-=count
              ...
              count=out_count
              satellite_queue[current_loop_id][satellite_id][key]-=out_count
              out_count=0
              send(key,count,satellite_id,i[0],epoch,now,num_satellite,current_loop_id,satellite_queue,ground_station_queue,packet_droped_table,satellites,ground_stations,isl_packet_drop_rate,gsl_packet_drop_rate,time_step)
      ########################
      # 卫星保存数据到下一个时刻
      ...
      for key in list(satellite_queue[current_loop_id][satellite_id].keys()):
        satellite_queue[current_loop_id+1][satellite_id][key]=satellite_queue[current_loop_id+1][satellite_id].get(key,0)+satellite_queue[current_loop_id][satellite_id][key]
        del satellite_queue[current_loop_id][satellite_id][key]
    ...
    for ground_station_id in range(0,num_ground_station):
      ...
      Queue_size=get_size(ground_station_queue[current_loop_id][ground_station_id])
      
      ground_station_queue_len[current_loop_id]+=Queue_size

      total_out_packets=ground_station_to_cloud_bandwidth[ground_station_id]
      
      for key in list(ground_station_queue[current_loop_id][ground_station_id].keys()):
        if total_out_packets==0:
          break
        if ground_station_queue[current_loop_id][ground_station_id][key]<=total_out_packets:
          packet=ground_station_queue[current_loop_id][ground_station_id][key]
          del ground_station_queue[current_loop_id][ground_station_id][key]
          total_out_packets-=packet
          data_center_queue[current_loop_id+1][key]=data_center_queue[current_loop_id+1].get(key,0)+packet
      
      for key in list(ground_station_queue[current_loop_id][ground_station_id].keys()):
        ground_station_queue[current_loop_id+1][ground_station_id][key]=ground_station_queue[current_loop_id+1][ground_station_id].get(key,0)+ground_station_queue[current_loop_id][ground_station_id][key]
        del ground_station_queue[current_loop_id][ground_station_id][key]
